...
void print_sizes(int * sizes, int * sizes_new, int * local_sizes,int rank){
	MPI_Barrier( MPI_COMM_WORLD);
		if(rank == 0 ){
...
void print_solution(int * sizes_new, int * local_sizes, MPI_Datatype &C_Block_Type, MatrixXd &C, int rank, int size,MPI_Comm Comm_Cart){
