...
int main (int argc, char **argv)
{
	...
	//MPI Init and Comm_Cart creationy
	MPI_Init(&argc, &argv);
	int size;
	int rank;
	MPI_Status status;
	...
	*/
	...
	for (int t=0; t<t_max; ++t){
		...
		//----------------NON BLOCKING GHOST CELL DISTRIBUTION ------------------------
		//-----------------------------------------------------------------------------------------------
		...
		if (mycoords[0]%2==0){
			...
			MPI_Irecv(&u.data_[u.num_x()-1],1,LR_TYPE,right,lr_tag,Comm_Cart, &reqs[0]);  //recieve left boundary from right as right boundary
			MPI_Isend(&u.data_[u.num_x()-2],1,LR_TYPE, right, lr_tag,Comm_Cart, &reqs[1]); //send rigt bound to right	
			MPI_Irecv(&u.data_[0],1,LR_TYPE,left,lr_tag,Comm_Cart, &reqs[2]); //recieve right boundary from left as left boundary
			MPI_Isend(&u.data_[1],1,LR_TYPE,left,lr_tag,Comm_Cart,&reqs[3]); //send left bound to left
		}
		...
		if (mycoords[1]%2==0){
			MPI_Isend(&u.data_[u.num_xy()],1,BT_TYPE,top,bt_tag,Comm_Cart,&reqs[4]); //send left bound to left
			MPI_Irecv(&u.data_[0],1,BT_TYPE,top,bt_tag,Comm_Cart, &reqs[5]); //recieve right boundary from left as left boundary
			MPI_Isend(&u.data_[u.num_xy()*(u.num_z()-2)],1,BT_TYPE, bottom, bt_tag,Comm_Cart, &reqs[6]); //send right bound to right
			MPI_Irecv(&u.data_[u.num_xy()*(u.num_z()-1)],1,BT_TYPE,bottom,bt_tag,Comm_Cart, &reqs[7]);  //recieve left boundary from right as right boundary		
		}
		else {
			MPI_Irecv(&u.data_[u.num_xy()*(u.num_z()-1)],1,BT_TYPE,bottom,bt_tag,Comm_Cart, &reqs[4]);  //recieve left boundary from right as right boundary	
			MPI_Isend(&u.data_[u.num_xy()*(u.num_z()-2)],1,BT_TYPE, bottom, bt_tag,Comm_Cart, &reqs[5]); //send right bound to right
			MPI_Irecv(&u.data_[0],1,BT_TYPE,top,bt_tag,Comm_Cart, &reqs[6]); //recieve right boundary from left as left boundary
			MPI_Isend(&u.data_[u.num_xy()],1,BT_TYPE,top,bt_tag,Comm_Cart,&reqs[7]); //send left bound to left
		}
		...
		if (mycoords[2]%2==0){
			MPI_Isend(&u.data_[u.num_x()],1,FB_TYPE,front,fb_tag,Comm_Cart,&reqs[8]); //send left bound to left
			MPI_Irecv(&u.data_[0],1,FB_TYPE,front,fb_tag,Comm_Cart, &reqs[9]); //recieve right boundary from left as left boundary
			MPI_Isend(&u.data_[u.num_xy()-2*u.num_x()],1,FB_TYPE, back, fb_tag,Comm_Cart, &reqs[10]); //send right bound to right
			MPI_Irecv(&u.data_[u.num_xy()-u.num_x()],1,FB_TYPE,back,fb_tag,Comm_Cart, &reqs[11]);  //recieve left boundary from right as right boundary		
		}
		else {
			MPI_Irecv(&u.data_[u.num_xy()-u.num_x()],1,FB_TYPE,back,fb_tag,Comm_Cart, &reqs[11]);  //recieve left boundary from right as right boundary
			MPI_Isend(&u.data_[u.num_xy()-2*u.num_x()],1,FB_TYPE, back, fb_tag,Comm_Cart, &reqs[10]); //send right bound to right
		...
		if (t==0) {
			...
			u_old=u; //needed to have ghost cells available in u_old
		}
		...
		//update INTERIOR local matrix (timestep)---------------------------
		//if(rank==0) cout << "compute timestep update: " << t << endl;
		double f = 0;
		//cout << "i am rank " << rank << "with f=" << f << endl;
		for(unsigned i=2; i<(n[0]); i++){
		...
		for(unsigned i=1; i<=n[0]; i+=(n[0]-1)){
		...
		for(unsigned i=1; i<=(n[0]); i++){
			for(unsigned j=1;j<=(n[1]); j+=(n[1]-1)){
				for(unsigned k=1; k<=(n[2]); k++){
					f=1./dx[0]*(-2.*u(i,j,k)+u(i+1,j,k)+u(i-1,j,k));
					f+=1./dx[1]*(-2.*u(i,j,k)+u(i,j+1,k)+u(i,j-1,k));
					f+=1./dx[2]*(-2.*u(i,j,k)+u(i,j,k+1)+u(i,j,k-1));
		...
		for(unsigned i=1; i<=(n[0]); i++){
			for(unsigned j=1;j<=(n[1]); j++){
				for(unsigned k=1; k<=(n[2]); k+=(n[2]-1)){
					f=1./dx[0]*(-2.*u(i,j,k)+u(i+1,j,k)+u(i-1,j,k));
					f+=1./dx[1]*(-2.*u(i,j,k)+u(i,j+1,k)+u(i,j-1,k));
					f+=1./dx[2]*(-2.*u(i,j,k)+u(i,j,k+1)+u(i,j,k-1));
					//cout << "f = " << f << endl; 
