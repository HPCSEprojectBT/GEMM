...
int main (int argc, char **argv)
{
	...
	int world_dim=3;
	int dims[3]={0,0,0};
	int N = 4; //target global number of DOFS in each direction
	...
	double init_heat = 10000000000;
	int print = 100;
	//--------------------------------------------------------

	//MPI Init and CartComm creation
	MPI_Init(&argc, &argv);
	int size;
	int rank;
	MPI_Status status;
	...
	//generate initial local matrizes 
	for (int k=0; k<n[2]; ++k) {
		for (int j=0; j<n[1]; ++j) {
			for (int i=0; i<n[0]; ++i) {
				if (rank == 14 && i==1 && j ==1 && k==1) u(i,j,k) = init_heat; //inital heat field
				else u(i,j,k) = 0;
			}
		}
	}
	...
	for (int t=0; t<t_max; ++t){
		...
		if(t<10 || t%print==0){
			...
			MPI_Barrier( MPI_COMM_WORLD );
			if(rank==0) cout << "Local vectors written" << endl;
			...
			MPI_Barrier( MPI_COMM_WORLD );
			if(rank==0) cout << "start gathering:" << endl;
