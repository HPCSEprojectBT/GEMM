...
int main (int argc, char** argv)
{
	...
	for(int files =0; files<size; ++files){
		...
		if(rank == files) {
			char filename[160];
			sprintf(filename, "cout_thread_%u",files);
			outfiles[files].open(filename, ios::app);
			//outfiles[files] << "initial A,B,C:" <<endl << A << endl << B << endl << C << endl;

			outfiles[files] << "A_local after scatter: " << endl;
			outfiles[files] << endl;
			for(unsigned i=0; i<n_loc; i++){
				for(unsigned j=0; j<n_loc; j++){
					outfiles[files] << A_loc[i][j] << "\t";
				}
				outfiles[files]<< endl;
			}
			outfiles[files] << endl << endl;
		}
		MPI_Barrier(MPI_COMM_WORLD);
	}
	...
	for(int files =0; files<size; ++files){
		MPI_Barrier(MPI_COMM_WORLD);
		if(rank == files) {
			outfiles[files] << "B_local after scatter: " << endl;
			outfiles[files] << endl;
			for(unsigned i=0; i<n_loc; i++){
				for(unsigned j=0; j<n_loc; j++){
					outfiles[files] << B_loc[i][j] << "\t";
				}
				outfiles[files]<< endl;
			}
			outfiles[files] << endl << endl;
	...
	for(int files =0; files<size; ++files){
		MPI_Barrier(MPI_COMM_WORLD);
		if(rank == files) {
			outfiles[files] << "A_local after init send: " << endl;
			outfiles[files] << endl;
			for(unsigned i=0; i<n_loc; i++){
				for(unsigned j=0; j<n_loc; j++){
					outfiles[files] << A_loc[i][j] << "\t";
	...
	for(int files =0; files<size; ++files){
		MPI_Barrier(MPI_COMM_WORLD);
		if(rank == files) {
			outfiles[files] << "B_local after init send: " << endl;
			outfiles[files] << endl;
			for(unsigned i=0; i<n_loc; i++){
				for(unsigned j=0; j<n_loc; j++){
					outfiles[files] << B_loc[i][j] << "\t";
	...
	for(int iterations=0; iterations < number_of_steps; ++iterations){
		...
		for(int files =0; files<size; ++files){
			MPI_Barrier(MPI_COMM_WORLD);
			if(rank == files) {
				outfiles[files] << "A_local before step "<< iterations+1 << endl;
				outfiles[files] << endl;
				for(unsigned i=0; i<n_loc; i++){
					for(unsigned j=0; j<n_loc; j++){
						outfiles[files] << A_loc[i][j] << "\t";
		...
		for(int files =0; files<size; ++files){
			MPI_Barrier(MPI_COMM_WORLD);
			if(rank == files) {
				outfiles[files] << "B_local before step "<< iterations+1 << endl;
				outfiles[files] << endl;
				for(unsigned i=0; i<n_loc; i++){
					for(unsigned j=0; j<n_loc; j++){
						outfiles[files] << B_loc[i][j] << "\t";
