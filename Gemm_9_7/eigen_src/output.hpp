...
void print_abc(MatrixXf &A,MatrixXf &B, MatrixXf &C_step, int size, int rank){
	for(unsigned i=0; i< size; i++){
		MPI_Barrier( MPI_COMM_WORLD);
		if(i==rank){
			cout << "i am rank " << rank << endl;
...
void print_solutions_easy(int * mycoords, int * dims, MatrixXf &C,int rank){
...
void gather_Matrix(vector<int> sizes_new, vector<int> local_sizes, int offset, MPI_Datatype A_Block_Type, MatrixXf A, int rank, int size, MPI_Comm Comm_Cart, string name){
		MatrixXf A_glob_temp (sizes_new[0+offset],sizes_new[1+offset] );
		MatrixXf A_glob (sizes_new[0+offset],sizes_new[1+offset] );
		...
		if (rank ==0){
				for (int p=0; p<size; ++p)
				{
						...
						MatrixXf local_block(local_sizes[0+offset],local_sizes[1+offset]);
...
void print_solution(vector<int> & sizes_new, vector<int> & local_sizes, MPI_Datatype &C_Block_Type, MatrixXf &C, int rank, int size,MPI_Comm Comm_Cart){
...
void print_matrix_distribution(int rank, int size, MatrixXf & A_loc, MatrixXf & A_glob) {
