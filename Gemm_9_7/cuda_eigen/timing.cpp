...
int main (int argc, char** argv)
{
		...
		int n_runs;
		n = atoi(argv[1]);
		sprintf(timing_file,"timing_cuda_eigen_%d.csv",atoi(argv[2]));
		n_runs = atoi(argv[3]);

#ifdef VERIFY
		sprintf(output_file,"output_thread_%d",rank);
		ofstream out[size];
		out[rank].open(output_file);
#endif //VERIFY
		...
		for (int runs =0; runs < n_runs; runs ++){
				...
				for(int iterations=0; iterations < number_of_steps; ++iterations){
						MatMul(&A_loc.data()[0], &B_loc.data()[0], &C_loc.data()[0],n_loc);
