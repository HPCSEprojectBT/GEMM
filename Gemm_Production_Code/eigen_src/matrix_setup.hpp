...
void scatter_matrix(int rank, int m, int n_loc, MatrixXf & A_glob, MatrixXf & A_loc, MPI_Comm & Comm_Cart, MPI_Status & status)
{
		//distribute global matrices over the ranks -------------------------
	
		if (rank ==0) {
				MatrixXf A_temp(n_loc,n_loc);
				for (int i=0; i<m; ++i){
						for (int j=0; j<m; ++j) {
							...
							if(recv_cart_rank !=0){	
								MPI_Send(&A_temp.data()[0],	1, 	Matrix_Block_Type,	recv_cart_rank,	77,	Comm_Cart);
							}
							else{
				...
				MPI_Recv(&A_loc.data()[0],1,Matrix_Block_Type,0,77,Comm_Cart,&status);	
		}		
//_END_ matrix distibution-------------------------------------------	
}

void gather_matrix(int rank, int m, int n_loc, MatrixXf & A_glob, MatrixXf & A_loc, MPI_Comm & Comm_Cart, MPI_Status & status)
{
		...
		if (rank ==0) {
				...
				for (int i=0; i<m; ++i){
					for (int j=0; j<m; ++j) {
						...
						//cout << "A_temp reads: \n " << A_temp << "\n sneds to " << recv_cart_rank << endl;
						if (send_cart_rank!=0){
							MPI_Recv(&A_temp.data()[0],1,Matrix_Block_Type,send_cart_rank,77,Comm_Cart,&status);
							//cout << "here" << endl;
				...
				MPI_Send(&A_loc.data()[0],1,Matrix_Block_Type,0,77,Comm_Cart);	
		}
		// _END_ gather ---------------------------------------
...
void initial_send(MPI_Comm &Comm_Cart,int rank, MatrixXf &A_tmp, MatrixXf &A,
					 MatrixXf &B_tmp, MatrixXf &B, MPI_Status &status, int size){
	int mycoords[2] ={0,0};
	...
	MPI_Sendrecv(&A.data()[0],1, Matrix_Block_Type,a_left_displ, lr_tag,
			&A_tmp.data()[0],1, Matrix_Block_Type, a_right_displ, lr_tag,
			Comm_Cart, &status);

	// Sends initial values of B to the top
	MPI_Sendrecv(&B.data()[0],1, Matrix_Block_Type,b_top_displ, bt_tag,
			&B_tmp.data()[0],1, Matrix_Block_Type, b_bot_displ, bt_tag,
			Comm_Cart, &status);
